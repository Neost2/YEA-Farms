...
const apiUrl = `https://api.openweathermap.org/data/2.5/weather?q=Elgin,AZ&appid=${apiKey}`;

fetch(apiUrl)
    .then(response => response.json())
    .then(data => {
        const weatherMap = document.getElementById('weather-map');
        weatherMap.innerHTML = `<h2>${data.name}</h2><p>Temperature: ${data.main.temp}°C</p>`;
    })
